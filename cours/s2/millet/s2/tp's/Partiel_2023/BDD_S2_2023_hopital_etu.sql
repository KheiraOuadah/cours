...
CREATE TABLE medecin(
   ...
   id_service INT,
   reponsable boolean, -- true(1) false(0) 
...
-- R1
SELECT COUNT(DISTINCT acte.id_medecin) AS medecins_avec_actes
FROM acte;
-- +---------------------+
-- | medecins_avec_actes |
-- +---------------------+
...
-- R2
SELECT COUNT(hospitalisation.id_patient) AS nbr_hospitalisation
FROM hospitalisation
WHERE id_service = (SELECT id_service FROM service WHERE nom LIKE 'ophtalmologie')
  AND (date_entree <= '2022-12-25' AND date_sortie >= '2022-12-25') ;
...
-- R3
SELECT COUNT(M.id_service), S.nom
FROM medecin M
JOIN service S
